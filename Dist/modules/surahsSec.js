...
import { settings, openSettingsSecBtn, openTranslationsSectionBtn } from "./header-preloader-settings.js";
import * as audioPlayer from "./audioPlayer.js";

const templates = {
   img: '<img src="./assets/muslimFamily.jpg" alt="family image" class="surah-section__family-img"/>',
   surah: {
      surahHeader: `<header class="surah-section__header fw-500">
         <div class="header__surah-names flex">
            <h2 class="header__eg-name">$\{surah-eg-name}</h2>
            <h2 class="ff-amiri header__ar-name">$\{surah-ar-name}</h2>
         </div>
         <div class="header__trn-and-btns flex">
            <div class="header__trn-container">
               <div>Translation By</div>
               <div class="header__trn-container__src">
                  <div class="header__translation-src">$\{surah-trn-src}</div>
                  <button class="header__change-translation-btn">(<span class="header__change-translation-btn__text">Change</span>)</button>
               </div>
            </div>
            <div class="header__btns-container">
               <button
                  class="header__audio-btn header__audio-btn--play text-accentText-2 $\{header-btn-state}"
               >
                  <span class="header__play-icon">
                     <i class="fa-solid fa-play"></i>
                     play
                  </span>
                  <span class="header__pause-icon">
      ...
      basmala: `<div class="surah-section__basmala">
      <svg
         baseProfile="tiny"
         ...
         viewBox="0 0 176 36"
         xml:space="preserve"
         class="surah-section__basmala__icon"
         aria-hidden="true"
         focusable="false"
      >
         <switch>
      ...
      <p class="sr-only">بِسْمِ اللَّهِ الرَّحْمَٰنِ الرَّحِيمِ</p>
   </div>`,
      versesContainer: `<div class="surah-section__verses"></div>`,
      preloadedVersesContainer: `<div class="surah-section__preloaded-verses">$\{content}</div>`,
      verse: `<div class="flex verse__header">
            <span class="verse__key ff-roboto fw-500" 
               >$\{verse-key}</span
            >
            <ul class="verse__features flex">
               <li>
                  <button
                     class="verse__audio-btn verse__feature"
                     aria-label="play verse audio"
                     title="play"
                  >
                     <svg
                        class="verse__play-icon icon"
                        viewBox="0 0 24 24"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                        aria-hidden="true"
               ...
               <li>
                  <button class="verse__bookmark-btn verse__feature" title="bookmark">
                     <svg
                        class="icon"
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 24 24"
               ...
               <li>
                  <button class="verse__copy-btn verse__feature" title="copy">
                     <svg
                        class="icon"
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 24 24"
                     ...
                     <span class="verse__copy-btn__copiedText">copied!</span>
                  </button>
               </li>
            </ul>
         </div>
         <div class="verse__content flow">
            <div class="verse__ar-txt ">
            $\{verse-world-btns}
               <p class="sr-only verse-sr-txt">$\{verse-sr-txt}</p>
            </div>
            <div class="verse__eg-txt">
            $\{verse-eg-txt}
            </div>
         </div>
      `,
      preloadedVerse: `<div class="verse verse--preloaded">
      <div class="flex verse__header">
         <span 
            class="verse__key ff-roboto fw-500"
            data-id="7"
         ></span>
         <ul class="verse__features flex">
            <li>
               <button
                  class="verse__audio-btn verse__audio-btn--play verse__feature"
                  aria-label="play verse audio"
               ></button>
            </li>

            <li>
               <button class="verse__bookmark-btn verse__feature"></button>
            </li>
            <li>
               <button class="verse__copy-btn verse__feature"></button>
            </li>
         </ul>
      </div>
      <div class="verse__content flow">
         <div class="verse__ar-txt"></div>
         <div class="verse__eg-txt"></div>
      </div>
   </div>`,
      verseWordBtn: `<div class="verse__ar-txt__word-btn" data-id="$\{verse-world-id}" aria-hidden="true">
         <span style="font-family: $\{verse-world-ar-ff},sans-serif;">$\{verse-world-ar-txt}</span>
         <span class="fs-300 ff-openSans verse__ar-txt__word-trn">$\{verse-world-eg-txt}</span>
      </div>`,
   },
};
const mainContentEl = document.getElementById("main");
export const segments = {
   lastVisitedSurahWords: {
      data: [],
   },
   playerVerses: {
      data: [],
...
export function build(map, track) {
   if (mainContentEl.dataset.track != track) return;

   const versesData = map.get("textData").content.verses
      ? map.get("textData").content.verses
      : [map.get("textData").content.verse];
   const audioData = map.get("audioData").content.audio_files[0];
   const surahInfo = surahsInfo[versesData[0].chapter_id - 1];
   const surahSecEl = document.createElement("div");
   const surahHeaderContent = replaceStr(templates.surah.surahHeader, [
      ["${surah-eg-name}", surahInfo.englishName],
      ["${surah-ar-name}", surahInfo.name],
      ["${surah-trn-src}", settings.translationSrc.name],
      [
         "${header-btn-state}",
         surahInfo.number == audioPlayer.audioPlayerElement.dataset.surahId && !audioPlayer.audio.paused
            ? "header__audio-btn--pause"
            : "",
      ],
   ]);
   let next_page = map.get("textData").content.pagination
      ? map.get("textData").content.pagination.next_page
      : false;
   let surahHeaderAudioBtn;
   let surahHeaderChangeTrnBtn;
   const preloadedVersesContent = next_page
      ? templates.surah.preloadedVersesContainer.replace("${content}", templates.surah.preloadedVerse.repeat(10))
      : "";
   // we will add basmala only if the surah not alfatiha and the page not a bookmark page
   const basmalaContent = surahInfo.number != 1 &&  versesData.length != 1 ? templates.surah.basmala : "";

   audioData.reciterId = map.get("audioData").reciterId;
   // reset segments object
   segments.lastVisitedSurahWords.data = [];

   // setup text content of surah
   surahSecEl.classList.add("surah-section");
   surahSecEl.dataset.id = surahInfo.number;
   surahSecEl.innerHTML =
      templates.img + surahHeaderContent + basmalaContent + templates.surah.versesContainer + preloadedVersesContent;
   mainContentEl.innerHTML = "";
   mainContentEl.append(surahSecEl);
   addVerses(versesData, audioData);

   // setup lazy scroll to load more verses if next_page not none
   if (next_page) {
      const preloadedVersesConEl = mainContentEl.querySelector(".surah-section__preloaded-verses");
      /* 
         we want to observe preloadedVersesCon, if it showed on the viewport we will load more verses and stop observing until the data loaded.
         after data loaded if there is more verses should load for the current surah, we back to observe it, if not we just remove it
      */
      const observer = new IntersectionObserver(
         function (entries) {
            if (!entries[0].isIntersecting) return;
            let url = replaceStr(thirdParty.versesContentApi, [
               ["${surah_id}", surahInfo.number],
               ["${translationSrc_id}", settings.translationSrc.id],
               ["${page_id}", next_page],
            ]);
            let map = new Map().set("data", { url: url, content: null });
            // load verses
            thirdParty.handelData(map, (map) => {
               next_page = map.get("data").content.pagination.next_page;

               addVerses(map.get("data").content.verses, audioData);

               // if there is more verses should loaded we back to observe preloadedVersesContainer
               if (next_page) {
                  observer.observe(entries[0].target);
               } else {
                  // if not we just remove it
                  preloadedVersesConEl.remove();
               }
            });
            // stop observing
      ...
      observer.observe(preloadedVersesConEl);
   }
   ...
   surahHeaderAudioBtn.addEventListener("click", (_) => {
      ...
      if (isSurahPlaying) {
         ...
         surahHeaderAudioBtn.classList.add(pauseMode);
         if (activeVerseAudioBtn) activeVerseAudioBtn.classList.add("verse__audio-btn--pause");
         playAudio(surahInfo.number, audioData, {
            seconds:
               surahInfo.number == audioPlayer.audioPlayerElement.dataset.surahId
   ...
   // add event listener to change translation button
   surahHeaderChangeTrnBtn = surahSecEl.querySelector(".header__change-translation-btn");
   surahHeaderChangeTrnBtn.addEventListener("click", (_) => {
      openSettingsSecBtn.click();
      openTranslationsSectionBtn.click();
   });
   ...
   document.title = `Surah ${surahInfo.englishName} - Quran For You`
}

function addVerses(versesData, audioData) {
   const versesContainerEl = mainContentEl.querySelector(".surah-section__verses");
   const surahId = audioData.chapter_id;

   versesData.forEach((verseData) => {
      const verseEl = document.createElement("div");
      const verseKey = verseData.verse_key;
      const wordBtnsContent = verseData.words.reduce((wordsContent, wordData) => {
         let wordContent = replaceStr(templates.surah.verseWordBtn, [
            ["${verse-world-ar-txt}", wordData.code_v1],
            ["${verse-world-ar-ff}", addQcfFont(wordData.v1_page)],
            ["${verse-world-eg-txt}", wordData.translation.text],
            ["${verse-world-id}", wordData.id],
         ]);
         return wordsContent + wordContent;
      }, "");
      const verseContent = replaceStr(templates.surah.verse, [
         ["${verse-key}", verseKey],
         ["${verse-sr-txt}", verseData.text_imlaei],
         ["${verse-eg-txt}", verseData.translations[0].text],
         ["${verse-world-btns}", wordBtnsContent],
      ]);
      const verseAudio = audioData.verse_timings[verseData.verse_number - 1];

      // add verse to versesContainerEl

      // check the local storage if the current verse is bookmarked
      if (localStorage.bookmarks) {
         const bookmarksStorage = JSON.parse(localStorage.bookmarks);
         if (bookmarksStorage.some((bookmark) => bookmark.verseKey == verseKey))
            verseEl.classList.add("verse--bookmarked");
      }
      verseEl.classList.add("verse");
      verseEl.dataset.key = verseKey;
      verseEl.innerHTML = verseContent;
      versesContainerEl.append(verseEl);

      // add words segments of the current verse  to words segments array
      verseAudio.segments.forEach((wordTiming) => {
         // verseData.words[wordTiming[0] - 1].id : is word id for the current word timing
         segments.lastVisitedSurahWords.data.push({
            id: verseData.words[wordTiming[0] - 1].id,
            timestamp_from: wordTiming[1] / 1000,
            timestamp_to: wordTiming[2] / 1000,
         });
      });

      // set eventlistener to verse buttons
      ...
      playVerseBtn.addEventListener("click", (_) => {
         ...
         if (isVersePlaying) {
            ...
            playVerseBtn.classList.add(pauseMode);
            surahHeaderAudioBtn.classList.add("header__audio-btn--pause");
            playAudio(surahId, audioData, { key: verseKey });
         }
      });

      // copy btn
      const copyVerseTxtBtn = verseEl.querySelector(".verse__copy-btn");
      const copiedTextEl = copyVerseTxtBtn.querySelector(".verse__copy-btn__copiedText");
      copyVerseTxtBtn.addEventListener("click", (_) => {
         if (navigator.clipboard) {
            navigator.clipboard
               .writeText(verseData.text_imlaei)
               .then((_) => {
                  copiedTextEl.style.display = "block";
                  setTimeout((_) => (copiedTextEl.style.display = "none"), 1000);
               })
               .catch((_) => alert("Please allow Clipboard permission"));
         } else {
      ...
      bookmarkBtn.addEventListener("click", (_) => {
         ...
         if (isVerseElBookmarked) {
            ...
            localStorage.bookmarks = JSON.stringify(
               bookmarksStorage.filter((bookmarkData) => bookmarkData.verseKey != verseKey)
            );
            ...
            if (localStorage.bookmarks) {
               const bookmarksStorage = JSON.parse(localStorage.bookmarks);
               bookmarksStorage.unshift({ surahId: surahId, verseKey: verseKey });
               localStorage.bookmarks = JSON.stringify(bookmarksStorage);
            } else {
               localStorage.bookmarks = JSON.stringify([{ surahId: surahId, verseKey: verseKey }]);
            }
            verseEl.classList.add("verse--bookmarked");
         }
      });
...
function playAudio(surah_id, audioData, startTime) {
   ...
   if (surah_id != audioPlayer.audioPlayerElement.dataset.surahId) {
      ...
      audioData.verse_timings.forEach((verseAudio) => {
         if (startTime.key && verseAudio.verse_key == startTime.key)
            startTime.seconds = verseAudio.timestamp_from / 1000;
      ...
      audioPlayer.audioPlayerElement.dataset.surahId = surah_id;
      audioPlayer.audioPlayerElement.dataset.reciterId = audioData.reciterId;
      audioPlayer.seekSliderEl.value = 0;
      audioPlayer.seekSliderEl.parentElement.style.setProperty("--seek-before-width", 0);
      audioPlayer.currentTimeEl.textContent = audioPlayer.durationEl.textContent = "";
      audioPlayer.playAudioBtn.classList.remove("audioPlayer__play-btn--pause");
   ...
   if (startTime.seconds) {
      audioPlayer.audio.currentTime = startTime.seconds + 0.05;
   } else if (startTime.key) {
      segments.playerVerses.data.some((verseSegment) => {
         if (verseSegment.key == startTime.key) {
            audioPlayer.audio.currentTime = verseSegment.timestamp_from + 0.05;
            return true;
         }
      });
   }
...
function addQcfFont(page) {
   // if the browser support FontFaceSet and the qcf font for this page isn't already installed
   if ("fonts" in document) {
      if (!document.fonts.check(`400 0 p${page}`)) {
         let url = `url("./Qcf_font/woff2/p${page}.woff2") format("woff2"),
            url("./Qcf_font/woff/p${page}.woff") format("woff"),
            url("./Qcf_font/ttf/p${page}.ttf") format("truetype")`;
         let font = new FontFace("p" + page, url);
         font
            .load()
            .then((_) => document.fonts.add(font))
            .catch((err) => console.log(err.message));
      }
   } else if (!document.getElementById("qcf-styles-link")) {
      let link = document.createElement("link");
      link.rel = "stylesheet";
      link.href = "./css/qcf-styles.css";
      link.type = "text/css";
      link.id = "qcf-styles-link";
